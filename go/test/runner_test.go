...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("VISITORSTATISTICS_TEST_LIVE") == "TRUE" ||
		os.Getenv("VISITORSTATISTICS_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("VISITORSTATISTICS_TEST_EXPLAIN"); explain != "" {
