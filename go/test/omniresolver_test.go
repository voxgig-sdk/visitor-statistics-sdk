...
import (
	...
	omni "github.com/voxgig-sdk/visitor-statistics-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.VisitorStatisticsError); is {
				out := map[string]any{
					"name":    "VisitorStatisticsError",
...
func omniCtx(arg any, client *sdk.VisitorStatisticsSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.VisitorStatisticsSDK); is {
			client = live
