...
func TestVisitorArrivalEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.VisitorArrival(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.VisitorArrival(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		visitorArrivalRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.visitor_arrival")))
...
func visitor_arrivalBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("VISITOR_STATISTICS_TEST_VISITOR_ARRIVAL_ENTID")
	...
	idmapResolved := core.ToMapAny(env["VISITOR_STATISTICS_TEST_VISITOR_ARRIVAL_ENTID"])
