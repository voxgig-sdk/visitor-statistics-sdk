# visitor-statistics-cli

boru-driven command-line client **and** interactive REPL for the VisitorStatistics
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/visitor-statistics-cli)
make build

# 2. See usage (words, entities, env vars)
./visitor-statistics-cli --help

# 3. Provide credentials once, via the environment
export VISITOR_STATISTICS_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./visitor-statistics-cli list visitor_arrival

# 5. Override the API base URL for a single call
VISITOR_STATISTICS_BASE=https://api.example.com ./visitor-statistics-cli list visitor_arrival

# 6. No arguments -> interactive REPL
./visitor-statistics-cli
visitor-statistics> list visitor_arrival
visitor-statistics> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/visitor-statistics-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/visitor-statistics-cli list visitor_arrival
   ```
...
Run with no arguments to open a REPL (prompt `visitor-statistics>`). Each line is
evaluated as its own boru expression:

```text
$ ./visitor-statistics-cli
visitor-statistics> list visitor_arrival
visitor-statistics> /help
visitor-statistics> /quit
```
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `visitor_arrival`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/visitor-statistics-cli`. |
...
(`./visitor-statistics-cli <expr>`) and interactively (the REPL), and expressions compose the
