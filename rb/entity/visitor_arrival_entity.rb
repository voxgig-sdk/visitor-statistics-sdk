...
class VisitorArrivalEntity
  ...
  def make
    opts = @_entopts.dup
    VisitorArrivalEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = VisitorStatisticsHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [VisitorArrival, Hash] the current VisitorArrival data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = VisitorStatisticsHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of VisitorArrival fields)
  def match_get
  ...
  #
  # @param reqmatch [VisitorArrivalListMatch, Hash, nil] match filter (any subset of
  #   VisitorArrival fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<VisitorArrival>, Array] the matching VisitorArrival items; raises VisitorStatisticsError on failure
