...
require_relative 'VisitorStatistics_types'
...
class VisitorStatisticsSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = VisitorStatisticsHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = VisitorStatisticsHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, VisitorStatisticsFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    VisitorStatisticsUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = VisitorStatisticsHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = VisitorStatisticsHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = VisitorStatisticsHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => VisitorStatisticsError.new(
        "#{op}_allow",
  ...
  def raw_request(fetchargs = {})
    ...
    rescue VisitorStatisticsError => err
    ...
    if fetched.is_a?(Hash)
      status = VisitorStatisticsHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = VisitorStatisticsError.new(
        "graphql_error", "VisitorStatisticsSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = VisitorStatisticsSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
