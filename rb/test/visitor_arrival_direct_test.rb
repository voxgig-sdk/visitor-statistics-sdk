...
class VisitorArrivalDirectTest < Minitest::Test
  def test_direct_list_visitor_arrival
    setup = visitor_arrival_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-visitor_arrival", setup[:live] ? "live" : "unit")
    if _should_skip
...
def visitor_arrival_direct_setup(mockres)
  ...
  if live
    ...
    client = VisitorStatisticsSDK.new(merged_opts)
    return {
      client: client,
      calls: calls,
      live: true,
