...
class VisitorArrivalEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    base = VisitorStatisticsSDK.test(seed, nil)
    seen = base.VisitorArrival(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.VisitorArrival(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    setup = visitor_arrival_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["list"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "visitor_arrival." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    visitor_arrival_ref01_data = nil
    if visitor_arrival_ref01_data_raw.length > 0
...
def visitor_arrival_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "visitor_arrival", "VisitorArrivalTestData.json")
