...
//    only - a typed VisitorStatisticsContext there would make every ctx
...
final class VisitorStatisticsOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new VisitorStatisticsContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new VisitorStatisticsSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new VisitorStatisticsResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new VisitorStatisticsError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new VisitorStatisticsResponse($clean['response']);
    ...
    public static function err_from_map($m): ?VisitorStatisticsError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new VisitorStatisticsError(is_string($code) ? $code : '', $msg);
    }
