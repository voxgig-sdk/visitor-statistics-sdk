...
// VisitorStatistics SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class VisitorStatisticsTestFeature extends VisitorStatisticsBaseFeature
{
    ...
    public function init(VisitorStatisticsContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (VisitorStatisticsContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = VisitorStatisticsParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (VisitorStatisticsContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(VisitorStatisticsContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = VisitorStatisticsParam::call($ctx, $k);
