...
// VisitorStatistics SDK retry feature
...
class VisitorStatisticsRetryFeature extends VisitorStatisticsBaseFeature
{
    ...
    public function init(VisitorStatisticsContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (VisitorStatisticsContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(VisitorStatisticsContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(VisitorStatisticsContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
