...
class VisitorStatisticsSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = VisitorStatisticsHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = VisitorStatisticsHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!VisitorStatisticsFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, VisitorStatisticsFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return VisitorStatisticsUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = VisitorStatisticsHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = VisitorStatisticsHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = VisitorStatisticsHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new VisitorStatisticsError($op . "_allow",
                "VisitorStatisticsSDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = VisitorStatisticsHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new VisitorStatisticsError("graphql_error",
                "VisitorStatisticsSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    public function VisitorArrival($data = null)
    {
        ...
        if ($data === null) {
            if ($this->_visitor_arrival === null) {
                $this->_visitor_arrival = new VisitorArrivalEntity($this, null);
            }
            return $this->_visitor_arrival;
        }
        return new VisitorArrivalEntity($this, $data);
    }
    ...
    public static function test(?array $testopts = null, ?array $sdkopts = null): self
    {
        ...
        $sdk = new VisitorStatisticsSDK($sdkopts);
        $sdk->mode = "test";
        return $sdk;
    }
}
