...
//     (\`new VisitorStatisticsSDK(...)\` / \`VisitorStatisticsSDK.test(...)\` become
//     \`VisitorStatisticsSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'VisitorStatisticsSDK'
...
const TEST_SEED = {"entity":{"visitor_arrival":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new VisitorStatisticsSDK(...)\` and any
// \`VisitorStatisticsSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+VisitorStatisticsSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/VisitorStatisticsSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
