# visitor-statistics-mcp

[MCP](https://modelcontextprotocol.io) server exposing the VisitorStatistics SDK as
two agent tools — `visitor-statistics_list` and `visitor-statistics_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/visitor-statistics-mcp)
make build

# 2. Provide credentials via the environment
export VISITOR_STATISTICS_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user visitor-statistics \
  -- /absolute/path/to/visitor-statistics-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./visitor-statistics-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// visitor-statistics_list: first page of records
{ "entity": "visitor_arrival" }
{ "entity": "visitor_arrival", "query": { } }

// visitor-statistics_load: one record by id
{ "entity": "visitor_arrival", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/visitor-statistics-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user visitor-statistics \
     -- "$PWD"/dist/*/visitor-statistics-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `visitor-statistics_list` and `visitor-statistics_load` tools now appear
   in new sessions. Ask the agent to *"list visitor_arrival using visitor-statistics"*
   and it calls `visitor-statistics_list` with `{"entity":"visitor_arrival"}`.
...
```sh
export VISITOR_STATISTICS_APIKEY=sk_live_xxx            # API key
export VISITOR_STATISTICS_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `visitor-statistics_list` tool
...
### Call the `visitor-statistics_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `visitor-statistics_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `visitor-statistics_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `VISITOR_STATISTICS_APIKEY` | API key sent with every request. |
| `VISITOR_STATISTICS_BASE` | Optional override of the API base URL. |
...
```sh
./visitor-statistics-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"visitor-statistics_load","arguments":{"entity":"visitor_arrival","query":{"id":1}}}}'
```
