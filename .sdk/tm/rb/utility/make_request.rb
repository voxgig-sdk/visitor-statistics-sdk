...
module VisitorStatisticsUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = VisitorStatisticsResponse.new({})
    ...
    if fetch_err
      ...
      response = VisitorStatisticsResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = VisitorStatisticsResponse.new(fetched)
    else
