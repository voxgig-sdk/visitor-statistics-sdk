...
// VisitorStatistics SDK feature test
//
...
class FtCtrl extends VisitorStatisticsControl
{
...
class FtClient
{
    ...
    private VisitorStatisticsUtility $utility;

    public function __construct(VisitorStatisticsUtility $utility, array $options)
    {
    ...
    public function get_utility(): VisitorStatisticsUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (VisitorStatisticsContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public VisitorStatisticsContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (VisitorStatisticsContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = VisitorStatisticsConfig::make_feature($name);
    ...
    private static function buildUrl(VisitorStatisticsSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof VisitorStatisticsError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new VisitorStatisticsResponse($fetched);
        }
    ...
    private function fail(VisitorStatisticsContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (VisitorStatisticsContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
