...
class TestVisitorArrivalDirect:

    def test_should_direct_list_visitor_arrival(self):
        setup = _visitor_arrival_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-visitor_arrival", "live" if setup["live"] else "unit")
        if _skip:
...
def _visitor_arrival_direct_setup(mockres):
