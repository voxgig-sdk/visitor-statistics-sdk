# VisitorStatistics SDK test runner
...
class VisitorStatisticsTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            VisitorStatisticsTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = VisitorStatisticsTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = VisitorStatisticsTestRunner.getenv("VISITOR_STATISTICS_TEST_LIVE")
        override = VisitorStatisticsTestRunner.getenv("VISITOR_STATISTICS_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = VisitorStatisticsTestRunner.getenv(key)
        ...
        explain = VisitorStatisticsTestRunner.getenv("VISITOR_STATISTICS_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["VISITOR_STATISTICS_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if VisitorStatisticsTestRunner._test_control is not None:
            return VisitorStatisticsTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                VisitorStatisticsTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            VisitorStatisticsTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = VisitorStatisticsTestRunner.load_test_control()
...
def load_env_local():
    VisitorStatisticsTestRunner.load_env_local()


def env_override(m):
    return VisitorStatisticsTestRunner.env_override(m)


def entity_data(v):
    return VisitorStatisticsTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return VisitorStatisticsTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return VisitorStatisticsTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return VisitorStatisticsTestRunner.load_test_control()


def live_client_options():
    return VisitorStatisticsTestRunner.live_client_options()


def live_delay_ms():
    return VisitorStatisticsTestRunner.live_delay_ms()
