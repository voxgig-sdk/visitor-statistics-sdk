...
from visitorstatistics_sdk.visitorstatistics_types import (
    VisitorArrival,
...
class VisitorArrivalEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return VisitorArrivalEntity(self._client, opts)
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[VisitorArrival]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.VisitorArrival().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
