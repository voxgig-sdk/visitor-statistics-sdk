...
from visitorstatistics_sdk.core.control import VisitorStatisticsControl
...
from visitorstatistics_sdk.core.helpers import get_ctx_prop, to_map
...
class VisitorStatisticsContext:
    def __init__(self, ctxmap=None, basectx=None):
        ...
        # Ctrl
        self.ctrl = VisitorStatisticsControl()
        ...
        if isinstance(sp, VisitorStatisticsSpec):
            self.spec = sp
        elif basectx is not None:
        ...
        if isinstance(r, VisitorStatisticsResult):
            self.result = r
        elif basectx is not None:
        ...
        if isinstance(rp, VisitorStatisticsResponse):
            self.response = rp
        elif basectx is not None:
    ...
    def resolve_op(self, opname):
        ...
        if opname == "":
            return VisitorStatisticsOperation({})
    ...
    def make_error(self, code, msg):
        return VisitorStatisticsError(code, msg, self)
